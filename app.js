...
const AUTO_ZONE_BY_NOTE = new Map([
  [35, "bass"], [36, "bass"],
  [37, "tom3"], [38, "tom3"], [39, "tom3"], [40, "tom3"],
  [41, "tom1"], [42, "cymbalRod"], [43, "tom5"], [44, "cymbalRod"],
  [45, "tom2"], [46, "cymbalRod"], [47, "tom4"], [48, "tom4"],
  [49, "cymbalRod"], [50, "tom3"], [51, "cymbalRod"], [52, "cymbalRod"],
  [53, "cymbalRod"], [54, "cymbalRod"], [55, "cymbalRod"], [56, "cymbalRod"],
  [57, "cymbalRod"], [58, "cymbalRod"], [59, "cymbalRod"], [60, "tom3"],
  [61, "tom1"], [62, "tom4"], [63, "tom4"], [64, "tom2"], [65, "tom3"],
  [66, "tom5"], [67, "tom4"], [68, "tom2"], [69, "cymbalRod"], [70, "cymbalRod"],
  [71, "cymbalRod"], [72, "cymbalRod"], [73, "cymbalRod"], [74, "cymbalRod"],
  [75, "tom3"], [76, "tom3"], [77, "tom1"], [78, "tom5"], [79, "tom1"],
  [80, "cymbalRod"], [81, "cymbalRod"]
]);
...
function playTom(when, vel, note) {
  const freqMap = {
    41: 90, 43: 110, 45: 130, 47: 150, 48: 170, 50: 200,
    60: 210, 61: 160, 62: 190, 63: 175, 64: 125, 65: 220, 66: 145
  };
...
function playDrumSound(note, when, vel) {
  if (percussionInstrument) {
    percussionInstrument.start({ note, velocity: Math.max(1, Math.round(vel * 127)), time: when });
    return;
  }
...
const FAMILY_PATCHES = {
  piano: { wave: "triangle", sub: "sine", subRatio: 0.3, attack: 0.006, decay: 0.08, sustainRatio: 0.4, release: 0.2, filter: 3400 },
  chromatic: { wave: "triangle", sub: "sine", subRatio: 0.35, attack: 0.004, decay: 0.05, sustainRatio: 0.3, release: 0.15, filter: 4200 },
  organ: { wave: "square", sub: "sine", subRatio: 0.45, attack: 0.012, decay: 0.02, sustainRatio: 0.95, release: 0.04, filter: 3800 },
  guitar: { wave: "sawtooth", sub: "triangle", subRatio: 0.2, attack: 0.004, decay: 0.1, sustainRatio: 0.3, release: 0.15, filter: 2600 },
  bass: { wave: "sine", sub: "triangle", subRatio: 0.55, attack: 0.005, decay: 0.05, sustainRatio: 0.8, release: 0.2, filter: 1100 },
  strings: { wave: "sawtooth", sub: "sawtooth", subRatio: 0.5, attack: 0.09, decay: 0.15, sustainRatio: 0.85, release: 0.35, filter: 2800 },
  ensemble: { wave: "sawtooth", sub: "square", subRatio: 0.4, attack: 0.11, decay: 0.15, sustainRatio: 0.85, release: 0.35, filter: 2600 },
  brass: { wave: "sawtooth", sub: "square", subRatio: 0.3, attack: 0.045, decay: 0.1, sustainRatio: 0.8, release: 0.15, filter: 3600 },
  reed: { wave: "sawtooth", sub: "sine", subRatio: 0.3, attack: 0.035, decay: 0.08, sustainRatio: 0.78, release: 0.15, filter: 2400 },
  pipe: { wave: "sine", sub: "triangle", subRatio: 0.25, attack: 0.03, decay: 0.05, sustainRatio: 0.75, release: 0.2, filter: 4200 },
  lead: { wave: "square", sub: "sawtooth", subRatio: 0.25, attack: 0.005, decay: 0.04, sustainRatio: 0.7, release: 0.12, filter: 4600 },
  pad: { wave: "sawtooth", sub: "sine", subRatio: 0.4, attack: 0.18, decay: 0.2, sustainRatio: 0.9, release: 0.45, filter: 2200 },
  other: { wave: "triangle", sub: "sine", subRatio: 0.3, attack: 0.008, decay: 0.06, sustainRatio: 0.55, release: 0.15, filter: 3200 }
};
...
function loadSmplrModule() {
  if (!smplrModulePromise) {
    smplrModulePromise = import("https://cdn.jsdelivr.net/npm/smplr/dist/index.mjs");
  }
...
function loadMelodicInstrument(program) {
  if (melodicInstruments.has(program)) return Promise.resolve(melodicInstruments.get(program));
  if (melodicInstrumentsLoading.has(program)) return melodicInstrumentsLoading.get(program);
  const name = GM_INSTRUMENT_NAMES[program] || "acoustic_grand_piano";
  const promise = loadSmplrModule()
      .then(({ Soundfont }) => new Soundfont(audioCtx, { instrument: name, kit: SOUNDFONT_KIT, destination: masterGain }).load)
      .then(instrument => {
        melodicInstruments.set(program, instrument);
        return instrument;
      })
      .catch(err => {
        console.warn(`Falling back to synth for program ${program} (${name})`, err);
        return null;
      });
...
function loadPercussionInstrument() {
  ...
  percussionLoading = loadSmplrModule()
      .then(({ Soundfont }) => new Soundfont(audioCtx, { instrument: "percussion", kit: SOUNDFONT_KIT, destination: masterGain }).load)
      .then(instrument => {
...
function scheduleTrigger(note, vel, delaySeconds, isDrum, noteDuration, program) {
...
function loop() {
  ...
  while (nextNoteIndex < notesFlat.length && notesFlat[nextNoteIndex].time <= elapsed + lookahead) {
    ...
    scheduleTrigger(n.midi, n.velocity || 0.8, n.time - elapsed, n.isDrum, n.duration, n.program);
    nextNoteIndex++;
  }
...
function loadMidiFile(file) {
  ...
  reader.onload = () => {
    ...
    midi.tracks.forEach(track => {
      const isDrum = track.channel === 9 || (track.instrument && track.instrument.percussion);
      const program = track.instrument ? track.instrument.number : 0;
      track.notes.forEach(n => {
        notesFlat.push({ time: n.time, midi: n.midi, velocity: n.velocity, duration: n.duration, isDrum, program });
    ...
    songDrumNotes.forEach(note => {
      ...
      if (mapping[zoneId] && !mapping[zoneId].includes(note)) mapping[zoneId].push(note);
    });
...
function makeChip(note, label, zoneId) {
  ...
  chip.addEventListener("dragstart", e => {
    e.dataTransfer.setData("text/plain", JSON.stringify({ note, from: zoneId || "" }));
...
function wireEvents() {
  ...
  els.palette.addEventListener("dragleave", () => els.palette.classList.remove("drag-over"));
